...
/*Hack:A set of file names to remove duplicate files from a directory*/
static Hash_table *file_name_set = NULL;
...
static bool
dev_file_name_compare (void const *x, void const *y)
{
  const char *a = x;
  const char *b = y;
...
/* Hack:Return true if one of the hash_entries matches FILE, else inserts it.  */
	     ...
	     char const *dirname)
{
  ...
  char *absolute_name;
   ...
   //HACK: Ignore files with same name
   if (file_name_match(absolute_name))
   	return 0;
